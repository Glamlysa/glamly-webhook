...
app.get("/gift", (req, res) => {
  res.send([
    ...
    "  <h1>أدخلي رقم جوالك لعرض هديتك</h1>",
    "  <p>سنتعرف على هديتك برقم الجوال الذي استلمت عليه الرسالة</p>",
    ...
    "  if(!phone){ alert('من فضلك ادخلي رقم جوالك'); return; }",
  ...
  try {
    ...
    if (rows.length === 0) {
      return res.send([
        ...
        "</head><body><h2>لم نجد هدية بهذا الرقم</h2></body></html>"
      ].join("\n"));
    }
    const gift = rows[0];
